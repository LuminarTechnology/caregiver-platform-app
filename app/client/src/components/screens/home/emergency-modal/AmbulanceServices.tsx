...
export const AmbulanceServices = () => (
  <View className="px-5 py-4">
    <View className="flex-row items-center mb-3">
      <AmbulanceIcon/>
      <Text className="ml-2 font-semibold text-gray-900">Ambulance Services</Text>
    </View>
    <Text className="text-sm text-gray-600 mb-4">Private ambulance services</Text>
    ...
    {ambulances.map((ambulance, index) => (
      <View key={index} className="bg-white border border-gray-200 rounded-2xl p-4 mb-3 flex-row items-center justify-between">
        <View className="flex-row items-center flex-1">
          <AmbulanceIcon/>
          <Text className="ml-3 text-base text-gray-900">{ambulance.name}</Text>
        </View>
